...
import { Building2, Truck, UtensilsCrossed, Coffee, ShoppingCart, ArrowRight, ArrowLeft } from "lucide-react";
        ...
        <div className="flex items-center gap-2 mb-8 max-w-xs mx-auto">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${s <= step ? "bg-blue-600 text-white" : "bg-white/10 text-white/30"}`}>{s}</div>
              {s < 2 && <div className={`flex-1 h-px ${s < step ? "bg-blue-600" : "bg-white/10"}`} />}
            </div>
          ))}
        </div>
              ...
              {ROLES.map((r) => {
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer rounded-2xl p-6 border transition-all duration-200 select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/70 ${active ? "bg-blue-500/10 border-blue-500/60 shadow-lg shadow-blue-500/10" : "bg-white/[0.04] border-white/[0.09] hover:bg-white/[0.08] hover:border-white/[0.18]"}`}
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${active ? "bg-blue-600/30" : "bg-white/[0.08]"}`}>
                      <Icon className={`h-6 w-6 ${active ? "text-blue-400" : "text-white/60"}`} />
                    </div>
                    <div className={`text-lg font-extrabold mb-0.5 ${active ? "text-white" : "text-white/80"}`}>{r.label}</div>
                    <div className={`text-[10px] font-bold uppercase tracking-widest mb-2 ${active ? "text-blue-400" : "text-white/30"}`}>{r.sub}</div>
                    <div className="text-xs text-white/40 leading-snug">{r.desc}</div>
                  </div>
                );
              })}
            </div>
            <Button onClick={handleStep1} className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold text-white gap-2">
        ...
        {step === 2 && (
          <div className="bg-white/[0.04] border border-white/[0.08] backdrop-blur-xl rounded-3xl shadow-2xl p-8">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.05] border border-white/[0.08] mb-6">
              {selected && (() => { const Icon = selected.icon; return <Icon className="h-5 w-5 text-blue-400 shrink-0" />; })()}
              <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 mt-2">
                ...
                <Button type="submit" className="flex-1 h-12 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold text-white" disabled={loading}>
